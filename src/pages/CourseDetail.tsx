...
export default function CourseDetail() {
  ...
  return (
    <div className="min-h-screen bg-[#FEFDFB]">
      ...
      <main>
        {isLoading ? (
          <div className="pt-[140px] pb-20 text-center"><Loader2 className="w-8 h-8 text-[#16A34A] animate-spin mx-auto" /></div>
        ) : !course ? (
          ...
          <>
            <section className="pt-[100px] pb-12 bg-[#1B2A4A]">
              <div className="container-main">
                ...
                <span className="font-mono-accent text-[12px] tracking-[0.1em] text-[#16A34A] uppercase mb-3 block capitalize">{(course.category || '').replace('_', ' ')}</span>
                ...
                <div className="flex flex-wrap items-center gap-4 text-white/80 text-[13px]">
                  <span className="flex items-center gap-1.5"><Clock className="w-4 h-4 text-[#16A34A]" />{course.duration}</span>
                  <span className="capitalize">{course.mode}</span>
                  <span className="bg-white/10 rounded-full px-4 py-1.5"><span className="text-white/60">Offline</span> <span className="font-display text-[18px] font-semibold text-[#16A34A]">{formatFee(course.offlineFee || course.fee)}</span></span>
                  <span className="bg-white/10 rounded-full px-4 py-1.5"><span className="text-white/60">Online</span> <span className="font-display text-[18px] font-semibold text-[#16A34A]">{formatFee(course.onlineFee)}</span></span>
                </div>
              </div>
            </section>
              ...
              <div className="container-main grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div>
                      ...
                      <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        {splitList(course.highlights).map((t) => (
                          <li key={t} className="flex items-center gap-2 text-[14px] text-[#4A5568]"><CheckCircle className="w-4 h-4 text-[#16A34A] flex-shrink-0" />{t}</li>
                        ))}
                      </ul>
                    </div>
                  )}
                ...
                <aside className="space-y-4">
                  <div className="bg-white border border-[#E8EDF5] rounded-2xl p-6">
                    <div className="w-12 h-12 rounded-xl bg-[#EAF7EF] flex items-center justify-center mb-4"><GraduationCap className="w-6 h-6 text-[#16A34A]" /></div>
                    <div className="grid grid-cols-2 gap-3 mb-4">
                      <div className="bg-[#EAF7EF] rounded-lg px-3 py-2.5">
